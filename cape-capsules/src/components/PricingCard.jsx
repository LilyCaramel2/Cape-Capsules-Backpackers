...
const PricingCard = ({ title, price, period, features, isPopular = false }) => {
  return (
    <div className="bg-white border-2 border-gray-200 rounded-lg p-6 flex flex-col">
      {isPopular && (
        <div className="bg-island-red text-white text-sm font-bold px-4 py-1 rounded-full mb-4 text-center">
          Most Popular
        </div>
      )}
      ...
      <ul className="space-y-3 mb-6 flex-grow">
        {features.map((feature, index) => (
          <li key={index} className="flex items-start">
            <span className="text-hut-yellow mr-2">✓</span>
            <span>{feature}</span>
          </li>
        ))}
      </ul>
      ...
      <p className="text-xs text-center mt-4 text-gray-600">
        <span className="bg-hut-yellow bg-opacity-20 px-2 py-1 rounded">R500 refundable deposit applies</span>
      </p>
    </div>
  );
};
